...
function RunAccessDecision:_removeCardFromCorp(card)
    ...
    elseif self.slot == SLOT_CORP_ARCHIVES then
        local deck = self.state.board:deck(SLOT_CORP_ARCHIVES)
        deck:erase(card)
    end
end
...
function RunAccessDecision:accessedCards()
    ...
    if isSlotRemote(self.slot) then
        for i = 0, self.state.board:count(self.slot) - 1 do
            deck:append(self.state.board:card(self.slot, i))
        end
    elseif self.slot == SLOT_CORP_RND then
        local rnd_deck = self.state.board:deck(self.slot)
    ...
    elseif self.slot == SLOT_CORP_ARCHIVES then
        local archives_deck = self.state.board:deck(self.slot)
    ...
    elseif self.slot == SLOT_CORP_HQ then
        ...
        if count > 0 then
            ...
            deck:append(self.state.board:card(SLOT_CORP_HAND, idx))
        end
    end
